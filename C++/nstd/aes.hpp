...
{
    /**
     * @brief AES (Advanced Encryption Standard) encryption and decryption class.
     */
    class aes
    {
    public:
    ...
    private:
        ...
        void inverseMixColumns(std::uint8_t state[4][4]) noexcept
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                state[0][j] = (0x0e * a[0]) ^ (0x0b * a[1]) ^ (0x0d * a[2]) ^ (0x09 * a[3]);
                state[1][j] = (0x09 * a[0]) ^ (0x0e * a[1]) ^ (0x0b * a[2]) ^ (0x0d * a[3]);
                state[2][j] = (0x0d * a[0]) ^ (0x09 * a[1]) ^ (0x0e * a[2]) ^ (0x0b * a[3]);
                state[3][j] = (0x0b * a[0]) ^ (0x0d * a[1]) ^ (0x09 * a[2]) ^ (0x0e * a[3]);
            }
        }
