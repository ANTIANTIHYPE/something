...
using namespace nstd::endianness;

void testEndianness()
{
    // Test 16-bit conversions
    std::uint16_t value16 = 0x1234;
    std::uint16_t net16 = htons(value16);
    std::uint16_t host16 = ntohs(net16);
    ...
    std::uint32_t net32 = htonl(value32);
    std::uint32_t host32 = ntohl(net32);
    ...
    std::uint64_t net64 = htonll(value64);
    std::uint64_t host64 = ntohll(net64);
