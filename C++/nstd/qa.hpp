...
public:
     ...
     * 
     * @throws std::invalid_argument if the size of the states vector is not a power of 2.
     */
    template <typename T>
    inline void hadamard(std::vector<std::complex<T>>& states)
    {
        ...
        std::vector<std::complex<T>> new_states(size);

        for (std::size_t i = 0; i < size; ++i)
        {
            std::size_t partner = i ^ (size / 2);
     ...
     */
    QuantumArray(QuantumArray&& other) noexcept : states(std::move(other.states)), size(other.size) { other.size = 0; }
